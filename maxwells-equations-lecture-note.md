...
> **Formula notation:** Equations are shown in fenced `text` blocks so they remain fully visible in any Markdown viewer. Backslash commands such as `\nabla` and `\mathbf{E}` are standard mathematical notation: `\nabla` means gradient/divergence/curl operator and `\mathbf{E}` means the vector electric field.
